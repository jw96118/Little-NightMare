...
void Engine::CAniCtrl::Set_AnimationSet(const _uint& iIndex,const _double& aniCount)
{
	...
	m_pAniCtrl->KeyTrackEnable(m_iCurrentTrack, FALSE, m_fAccTime + aniCount);
...
string CAniCtrl::Get_AnimationName(int i)
{
	LPD3DXANIMATIONSET		pAS = NULL;

	// 인덱스 값에 해당하는 애니메이션 셋을 얻어는 함수
	if (m_pAniCtrl->GetAnimationSet(i, &pAS))
	{
		return "";
	}
	return pAS->GetName();
...
_float CAniCtrl::Get_AniPercent()
{
	...
	return tTrackInfo.Position / m_dPeriod;
}
