...
_bool Engine::CFrustum::Is_InFrustum(const _vec3* pPos)
{
...
_bool Engine::CFrustum::Is_InFrustum(const _vec3* pPos, _float fRadius)
{
	_float	fDistance = 0.f;
...
_bool Engine::CFrustum::Is_InFrustum_ForObject(const _vec3* pWorldPos, _float fRadius /*= 0.f*/)
{
	Ready_Frustum();
...
void Engine::CFrustum::Is_InFrustum_ForTerrain(const _vec3* pVtxPos, 
												const _ulong& dwCntX, 
												const _ulong& dwCntZ, 
												INDEX32* pIndex, 
												_ulong* pTriCnt,
...
{
	Ready_Frustum();
	ZeroMemory(pIndex, sizeof(INDEX32) * (dwCntX - 1) * (dwCntZ - 1) * 2);
	...
	*pTriCnt = 0;
	...
	else
	{
		...
		for (_ulong i = 0; i < dwCntZ - 1; ++i)
		{
			...
			{
				_ulong	iIndex = i * dwCntX + j;

				// 오른쪽 위
				bIsIn[0] = Is_InFrustum(&pVtxPos[iIndex + dwCntX]);
				bIsIn[1] = Is_InFrustum(&pVtxPos[iIndex + dwCntX + 1]);
				bIsIn[2] = Is_InFrustum(&pVtxPos[iIndex + 1]);

				if (true == bIsIn[0] || true == bIsIn[1] || true == bIsIn[2])
				{
					pIndex[dwTriCnt]._0 = iIndex + dwCntX;
					pIndex[dwTriCnt]._1 = iIndex + dwCntX + 1;
				...
				// 왼쪽 아래
				bIsIn[0] = Is_InFrustum(&pVtxPos[iIndex + dwCntX]);
				bIsIn[1] = Is_InFrustum(&pVtxPos[iIndex + 1]);
				bIsIn[2] = Is_InFrustum(&pVtxPos[iIndex]);

				if (true == bIsIn[0] || true == bIsIn[1] || true == bIsIn[2])
				{
					pIndex[dwTriCnt]._0 = iIndex + dwCntX;
					pIndex[dwTriCnt]._1 = iIndex + 1;
					pIndex[dwTriCnt]._2 = iIndex;
					++dwTriCnt;
				}
